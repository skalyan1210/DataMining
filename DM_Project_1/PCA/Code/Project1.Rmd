...
```{r}
#taking all the variables except disease column
...
```{r}
final_dataset <- final(normalize(dataset1),2,eigen_vectors) #calling the function that we defined
...
#plotting PCA
library("ggplot2")
ggplot(final_dataset, aes(X1,X2, colour= V3))+geom_point()+xlab("PC-1")+ylab("PC-2")+ggtitle("PCA")+labs(colour = "Diseases")

```
#SVD Implementation
```{r}
dataset2 <- dataset
svd <- svd(dataset2[,1:ncol(dataset2)-1])
svd
